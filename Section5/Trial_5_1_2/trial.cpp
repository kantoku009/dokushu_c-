...
public:
	stopwatch();				//コンストラクタ.
	stopwatch(clock_t i_clock);	//コンストラクタ.
	~stopwatch();				//デストラクタ.
 ...
 * @brief ストップウォッチクラス コンストラクタ.
 */
stopwatch::stopwatch(clock_t i_clock)
{
	this->m_start = (double)i_clock / CLOCKS_PER_SEC;
}
...
int main()
{
	...
	stopwatch cStopWatch(theClock);	//ストップウォッチクラス生成.
